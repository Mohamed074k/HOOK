...
const ProfileSupported = ({ posts, isLoading }) => {
  if (isLoading) return <div className="space-y-6">{Array(3).fill(0).map((_, i) => <PostSkeleton key={i} />)}</div>;

  if (!posts || posts.length === 0) {
    return (
      <div className="text-center py-12">
        <HandHeart size={48} className="mx-auto text-white/5 mb-4" />
        <p className="text-[#a3cbf2]/50 font-medium">No supported complaints yet.</p>
      </div>
    );
  }
