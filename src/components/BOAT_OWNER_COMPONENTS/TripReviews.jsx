// src/components/BOAT_OWNER_COMPONENTS/TripReviews.js
...
const TripReviews = ({ trip, onBack, animate }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        setLoading(true);
        const { data } = await apiClient.get(`/api/Reviews/allroles/trip/${trip.id}`);
        setReviews(data || []);
      } catch (error) {
  ...
  // حساب النسبة المئوية لكل نجمة لترتيب (5, 4, 3, 2, 1)
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-[#a3cbf2]/50">Loading reviews for {trip.title}...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 w-full pb-12">
      ...
      <div className={`transform transition-all duration-700 ease-out ${
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-[#cee5ff]">Trip Reviews</h1>
            <p className="text-[#a3cbf2]/50 text-sm mt-1">{trip.title}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {reviews.length === 0 ? (
          <div className="bg-[#002238] border border-white/5 rounded-2xl p-12 text-center transform transition-all duration-700 ease-out"
            ...
            <p className="text-[#a3cbf2]/40 text-sm">No reviews yet for this trip.</p>
          </div>
        ) : (
            ...
            >
              <div className="flex items-start justify-between mb-3 gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[#cee5ff] font-semibold group-hover:text-white transition-colors">{review.userName || "Guest"}</p>
