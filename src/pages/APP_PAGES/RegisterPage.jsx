...
// // ─── Controlled Floating Label Field ─────────────────────────────────────────
// const FloatingField = ({
...
//         style={{ background: "radial-gradient(ellipse, rgba(83,214,251,0.08) 0%, transparent 70%)" }}
//       />
...
//                 {GOVERNORATES.map((g) => (
//                   <option key={g} value={g} style={{ background: "#0E2132", color: "#DAE2FD" }}>
//                     {g}
//                   </option>
//                 ))}
//               </select>
...
//                 I agree to the{" "}
//                 <motion.span
//                   className="text-[#FFB77D] inline-block cursor-pointer"
...
//                 scale: 1.02,
//                 boxShadow: "0 0 32px rgba(83,214,251,0.45), 0 8px 24px rgba(0,138,167,0.3)",
//               } : {}}
...
const FishingBoatScene = () => {
  ...
  useEffect(() => {
    ...
    // Distant mountains / land silhouette points
    ...
    const draw = (t) => {
      ...
      const bw = Math.min(w * 0.38, 200); // boat width
      const bh = Math.min(h * 0.055, 32); // boat hull height
      ...
      // fishing line from tip — curves down with wind/sag
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.description || err?.response?.data?.message
              ...
              <motion.div className="flex items-center gap-2.5 pt-1" variants={itemVariants}>
                <div className="relative flex items-center justify-center flex-shrink-0">
                  <input type="checkbox" id="terms" checked={termsAccepted}
                    onChange={(e) => setTermsAccepted(e.target.checked)} disabled={isLoading}
                    className="peer w-4 h-4 rounded appearance-none border border-[#53D6FB] bg-transparent checked:bg-[#53D6FB] transition-all duration-300 cursor-pointer"
                  />
                  <div className="absolute text-[#003542] opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity duration-300">
                    <CheckCircle2 size={12} strokeWidth={3} />
